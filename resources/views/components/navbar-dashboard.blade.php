<nav @php $route=Route::currentRouteName() @endphp
        ...
        <nav>
            ...
            <ol class="flex flex-wrap pt-1 mr-12 bg-transparent rounded-lg sm:mr-16">
                ...
                <li class="text-size-sm pl-2 capitalize leading-normal text-slate-700 before:float-left before:pr-2 before:text-gray-600 before:content-['/']"
                    aria-current="page">{{ $route }}</li>
            </ol>
            <h6 class="mb-0 font-bold capitalize">{{ $route }}</h6>
        </nav>
            ...
            <ul class="flex flex-row justify-end pl-0 mb-0 list-none md-max:w-full">
                ...
                <li class="flex items-center px-4">
                    <form action="{{ route('logout') }}" method="post">
                        @csrf
                        <button type="submit" class="p-0 transition-all text-size-sm ease-nav-brand text-slate-500">
                            <i fixed-plugin-button-nav class="cursor-pointer fa fa-sign-out" aria-hidden="true"></i>
